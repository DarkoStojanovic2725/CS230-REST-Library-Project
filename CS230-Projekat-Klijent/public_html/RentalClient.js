...
// Model for Rental entity
    models.Rental = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.rental/",
        idAttribute: 'rentalId',
        defaults: {
            totalPrice: ""
        },
        toViewJson: function () {
            var result = this.toJSON(); // displayName property is used to render item in the list
            result.displayName = this.get('rentalId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.rental/';
            }
    ...
    // Collection class for Rental entities
    models.RentalCollection = Backbone.Collection.extend({
        model: models.Rental,
        url: "http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.rental/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.RentalCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-rental-list-itemis template identifier for item
                        templateName: '#tpl-rental-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.rental = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.rental,
                    // tpl-rental-details is template identifier for chosen model element
                    templateName: '#tpl-rental-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Rental();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-rental-details is a template identifier for chosen model element
                templateName: '#tpl-rental-details',
