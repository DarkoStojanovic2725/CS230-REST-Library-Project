...
// Model for Author entity
    models.Author = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.author/",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.author/';
            }
    ...
    // Collection class for Author entities
    models.AuthorCollection = Backbone.Collection.extend({
        model: models.Author,
        url: "http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.author/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.AuthorCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-author-list-itemis template identifier for item
                        templateName: '#tpl-author-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.author = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.author,
                    // tpl-author-details is template identifier for chosen model element
                    templateName: '#tpl-author-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Author();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-author-details is a template identifier for chosen model element
                templateName: '#tpl-author-details',
