...
// Model for Book entity
    models.Book = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.book/",
        idAttribute: 'bookId',
        defaults: {
            rentalPrice: "",
            availableNum: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('bookId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.book/';
            }
    ...
    // Collection class for Book entities
    models.BookCollection = Backbone.Collection.extend({
        model: models.Book,
        url: "http://localhost:8080/CS230-Projekat/webresources/com.cs230.projekat.book/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
            ...
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        ...
                        // tpl-book-list-itemis template identifier for item
                        templateName: '#tpl-book-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.book = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.book,
                    // tpl-book-details is template identifier for chosen model element
                    templateName: '#tpl-book-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Book();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-book-details is a template identifier for chosen model element
                templateName: '#tpl-book-details',
